...
exports.getOutgoingPayments = exports.getIncomingPayments = exports.getAllPayments = exports.addPayment = void 0;
const paymentService = __importStar(require("../services/payments/payments.service"));
...
const addPayment = async (req, res) => {
    ...
    const createPaymentModel = req.body;
    const paymentResult = await paymentService.addPayment(createPaymentModel);
    if ((0, result_1.isResultError)(paymentResult)) {
        res.status(500).json({ error: paymentResult.error });
        return;
    }
    res.json((0, controller_utils_1.ModelToResponseBodyMapper)(paymentResult.data));
};
...
const getIncomingPayments = async (req, res) => {
    if (!req.isAuthenticated()) {
        res.status(401).json({ error: 'Unauthorized' });
        return;
    }
...
const getOutgoingPayments = async (req, res) => {
    if (!req.isAuthenticated()) {
        res.status(401).json({ error: 'Unauthorized' });
        return;
    }
