...
export class DashboardRepository {
  async getDashboardMetricsAggregated(dateFilter?: DateFilter): Promise<Result<DashboardMetricsData>> {
    try {
      ...
      const totalExpenses =
        Number(results.find((result: GroupedResult) => result.type === 'outgoing')?._sum.amount) || 0;
