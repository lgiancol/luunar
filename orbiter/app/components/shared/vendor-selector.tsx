...
export function VendorSelector({
  ...
  if (loading) {
    return <div className={`h-10 animate-pulse rounded bg-gray-200 ${className}`} />;
  }
  ...
  return (
    <DataSelector
      ...
      showAdd={!!onAddVendor}
    >
      <DataSelector.SelectedItem>{(vendor: Vendor) => <div>{vendor.name}</div>}</DataSelector.SelectedItem>
